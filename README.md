...
|:---------------------------------------------:|:------------------:|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
|                    Golang                     |      Backend       | Основной язык для бекенда. Используем ввиду его хорошего соотношения производительности, поддерживаемости и кол-ва существующих инструментов. Также есть современные механизмы асинхронна из коробки. |
|                   Cassandra                   |      Backend       |                                  Основное хранилище постоянных данных на бекенде, используем ввиду встроенной возможности шардирования и высокой производительности.                                  |
|                     Redis                     |      Backend       |                                                                           in-memory хранилище со встроенным шардированием.                                                                            |
|                     Kafka                     |      Backend       |                          Высокопроизводительное, масштабируемое, отказоустойчивое решения для межсервисного взаимодействия, а также хранения несохраняемых на сервере данных                          |
|          Amazon S3 Express One Zone           |      Backend       |                                       Объектное хранилище для пользовательских файлов. Используем ввиду удобства, масштабируемости и высокой производительности                                       |
|                     Nginx                     |      Backend       |                                                                  Распространенное высокопроизводительное решение для прокси серверов                                                                  |
|                  Prometheus                   |      Backend       |                                                                                              Сбор метрик                                                                                              |
|                    Grafana                    |      Backend       |                                                                                     Отображение метрик и алармов                                                                                      |
|                    Gitlab                     |      Backend       |                                                                       Система контроля версий, дает больше контроля, чем GitHub                                                                       |
...
|                    Jaeger                     |      Backend       |                                                                  Трассировка для отладки ошибок и багов в микросервисной архитектуре                                                                  |
| Dual-Signal Transformation LSTM Network(DTLN) |   Backend&Client   |                                                                           Улучшение качество голосовых звонков и сообщений                                                                            |
|                  WebRtc+ICE                   |   Backend&Client   |                                                                               Поддержка голосовых звонков с обходом NAT                                                                               |
|                     React                     |       Client       |                                                                            Фреймворк от Meta для использования на клиенте.                                                                            |
|                    sql.js                     |       Client       |                                             Библиотека для использования SQLite в браузере, нужна для унификации способов хранения на различный клиентах.                                             |
...
# 9. Схема проекта
[Полная схема проекта](https://excalidraw.com/#json=mq6bw_39fgrXoq01UfkDA,NK9S0G4VwBaUCDyyi26ooA)

![схема проекта](img/Схема_проекта.png)

Будем использовать API Gateway - Backend for frontend для изолированности логики различных клиентов и для централизованной 
точки роутинга. Также будем микросервисы по бизнес слоям для изоляции логики и стандартизации взаимодействия 
микросервисов.

**Сервис message** - основной сервис, который используется для отправки и получения сообщений.
Все сообщения будут храниться в кафке, настроенной на автоматическое удалении, в случае если сообщению больше
30 дней. Кафка имеет два топика - один для личных сообщений, другой для групповых, так как логика удаления разная.
Для выдерживания такой нагрузки будет создано около 200 партиций на топик. Так как кафка способна писать 30к рпс на партицию
то для 4.5 млн рпс на запись требуется 4500/30 = 150. Для чтения это также удовлетворяет, так как кафка способна читать
500к рпс с партиции а рпс чтения требуемый 55млн итого требуется 110 партиций.

**Сервис file** - сервис для работы с файлами, метаданные также хранятся в кафке, так как временны, сами файлы хранятся
в обьектном хранилище. Кроме того, в случае, если файл это голосовое сообщение, используется ML для уменьшения шума

**Сервис auth** - сервис, отвечающий за авторизацию пользователей и хранение их прав. Авторизация будет производитс по
jwt-токенам, в редисе будет лежать рефреш, который обновляет jwt раз в 5 минут. 

**Сервис user** - сервис, который отвечает за работу с пользователями.

**Сервис group** - сервис, отвечающий за работу с группами. 

**Сервис call** - осуществляет соединение голосовых звонков Websoсket, а также содержать Stun и Turn сервера 
для соединения p2p клиентов между собой. Также на Turn серверах будет оптимизироваться обработка звонков с большим
количеством пользователей, например объединятся дорожки. Также есть ML обработка звонков, чтобы уменьшить количество
шума в них. 

**Сервис metrics** - сервис, который отвечает за сбор, хранение, отображение и отрисовку метрик в Grafana.

# 10. Обеспечение надежности

|           Уровень           |                                                                                                                                                                   Меры                                                                                                                                                                   |
|:---------------------------:|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
|         Уровень ДЦ          |                     Резервирование электропитания, линий связи, охлаждения<br/>Использование аварийных дизель генераторов для возможности реализации Graceful Shutdown<br/>Расположение ДЦ рядом с источниками электричества(ГЭС, АЭС), но далеко от потенциально аварийных обьектов(аэропорт) и вне городской черты                     | 
|         Уровень БД          |                                                                                                                                               Приведено в отдельной таблице для каждой БД                                                                                                                                                |
|      Уровень Сервисов       |                                                      Резервирование ресурсов(CPU, RAM), физических компонентов(сервера и диски). Для дисков в БД будет использоваться RAID 6, так как это повысит отказоустойчивость, кроме того при отказе одного диска система продлжит работать.                                                      | 
| Разработка и инфраструктура | Проведение codeReview, проведение тестирования(от юнитов до E2E и ручных), CI/CD с настроеным линтерами и прогоном всех юнитов, плюс сонар куб. Проведение учений(отключение одного ДЦ) GracefulShutdown, Расположение сервисов в различных зонах(стойка, ряд, зал, дц), что уменьшит область отказа, использование GracefullDegradation |
|        Наблюдаемость        |                                                                                                              Логирование, трассировка, мониторинг, алерты, профилирование. Поможет анализировать текущее состояние системы                                                                                                               |

Уровень БД

|    БД     |                                                   Меры                                                   |
|:---------:|:--------------------------------------------------------------------------------------------------------:|
| Cassandra |      Репликация с replication factor = 4. Три реплики располагаются в одном ДЦ, четвертая в другом       |
| Cassandra |                       Уровень согласования QUORUM (ждем ответ от 3 реплик(N/2+1))                        |
|   Kafka   | Для всех топиков используется replication factor = 4, так как через кафку идет основной поток информации |
|   Kafka   |  Гарантия доставки Exactly Once, чтобы сообщения не терялись. На уровне бэка будут отсеиваться реплики   |
|   Redis   |                    Две реплики в одном ДЦ и одна в другом, создание бэкапов раз в час                    |

Доп. паттерны:

|       паттерн        |                                                                                                                                                                                                                              Мера                                                                                                                                                                                                                              |
|:--------------------:|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
| Асинхронные паттерны |                                                                                                                                                                                                   Паттерн eventDriven - используется для обработки сообщений                                                                                                                                                                                                   | 
|     RateLimiting     |                                                                                                                                                                                             на уровне API Gateway будет использовать RL, чтобы защититься от ddos                                                                                                                                                                                              |
|        Retry         |           Retry реализован на API Gateway, с использованием 3 ретраев и подходом exponential backoff(экспоненциальное увеличение времени между попытками). Также будет использоваться retry budget - общее ограничение кол-ва попыток как процент от успешно выполненных запросов, что позволяет не генерировать лишние ретрае в случае аварии. Ретраются только подходящие запросы, 4хх, 2хх, 3хх не ретраются, также механизм мониторинга ретраев.           |
|   GracefulShutdown   | На уровне приложения будет обрабатываться сигнал SIGTERM, при получении которого начинается GracefulShutdown(Стопаем обработку новых запросов, завершаем старые, освобождаем ресурсы). Развертка в кубер, следовательно между отправками сигналов SIGTERM и SIGKILL поставим 45 секунд, так как неободимо дождаться завершения всего и удаления данных из Endpoint и компонентов ingress controller, kube-proxy, чтобы не направлять трафик на нерабочие поды. |


# 11. Расчет ресурсов

|          Сервис           |           RPS            |  CPU   | RAM, Гб | net Гбит/с |
|:-------------------------:|:------------------------:|:------:|:-------:|:----------:|
|    web && mobile nginx    | 59.580.013 + 122.248 CPS | 10'273 |  1'028  | 4'945'668  |
| web && mobile api gateway |        59.702.261        | 9'012  |    0    | 4'945'668  |
|           auth            |         199.008          |   50   |  0.61   |   0.023    | 
|           user            |        4'277'700         | 1'068  |   13    |    0.5     | 
|           call            |          9.000           |  113   |   14    |    1.1     |
|           group           |         300.000          |   75   |    1    |    0.04    | 
|      message.writer       |        4.332.984         | 54'162 |  6'611  |    0.51    | 
|      message.reader       |        55.351.280        | 13'837 |   169   |    6.6     |  
|        file.writer        |        1.896.080         |  632   |   62    |   1'158    |  
|        file.reader        |        24.868.825        | 8'290  |   810   |   15'178   |  

## Расчет сервисов 
Через ***web && mobile nginx***, а также ***web && mobile api gateway*** будут проходить все запросы. Общий RPC для запросов, требующих
нового tcp handshake = 122.222 + 10 + 16 = 122.248 CPS

Общий RPS для запросов, которые переиспользуют tcp соединение = 995.328+1.319.386+69.682+92.366+425.700+564.300+213.280+282.720+
106.640+141.360+796.262+29.686.184+55.744+2.078.235+340.560+12.696.550+170.624+6.261.200+85.312+3.180.600 = 59.580.013 RPS

Примем средний размер запроса 100Кб с учетом файлов. Тогда, согласно бенчам nginx для https 1 ядро = примерно 428 Cps (10Мб RAM). 
1 ядро = 5800 RPS(100мб RAM) без установления tcp соединения каждый раз. 

Тогда, количество ядер, которое требуется для обработки запросов:\
CPS: 122.248 / 428 = 286 ядер\
RPS: 59.580.013 / 5800 = 10.273 ядра\
Выбираем худший вариант - 10.273 ядра.

Суммарный трафик = 3.6 + 5 + 48 + 64 + 13.303 + 17.634 + 132.000 + 174.000 + 8.330 + 11.042 + 0.28 + 0 + 0.2 + 3 + 104 + 
40 + 1.426 + 10.400 + 388.000 + 106.000 + 3.930.000 + 6.664 + 242.000 = 4.945.668 Гб/с

Одна сетевая карта способна пропускать порядка 600 Гбит/с(теоретический максимум 800).
Тогда, необходимо: 4.945.668/600 = 8243 сетевые карты.

На api Gateway будет поступать http трафик, тогда, согласно бенчам nginx, 1 ядро обрабатывает 33.125 RPS.
Считаем, что api более нагружен, чем nginx и менее производительный, поэтому возьмем запас в 5 раз. 
Следовательно, необходимо: (59.580.013 + 122.248) * 5 / 33.125 = 9.012 ядер

Т.к. производительности веб фреймворков Go и C++ схожа, а реальная производительности и потребление RAM сильно зависит от
реализации приложения, предположим, что Go в 1.25 раз требовательнее к CPU и к RAM чем С++. Тогда таблица на основе
методических указаний потребления ресурсов на ядро.

|  Характеристика сервиса   |   RAM   | RPS  |
|:-------------------------:|:-------:|:----:|
|   Тяжелая бизнес логика   | 125 МБ  |  8   |
|   Средняя бизнес-логика   | 125 МБ  |  80  |
|      Легкое JSON API      | 12.5 МБ | 4000 |
| Потоковая передача данных | 100 МБ  | 3000 |

Будем считать, что все API, отдающие текст не превышают 1Кб. А средний размер у файла с учетом всех файлов 5Мб.

***auth*** - легко JsonAPI. Учтем, что каждый запрос раз в 5 минут приходит в сервис auth, чтобы обновить jwt токен. Тогда, доп
нагрузка на сервис auth помимо регистрации - 59.702.261 / 5 / 60 = 199.008 / 4000 = 50 ядер. RAM 50 * 12.5 / 1024 = 0.61 Гб
net = 199.008 * 1 / 1024 / 1024 / 8 = 0.023 Гбит/с 

***user*** - легкое JSON API. Получение пользователей происходит при создании и открытии групп, также когда пользователь только 
заходит и подгружает чаты. Было рассчитано выше. Примерно 2'138'850 RPS. Умножим на 2, так как считаем пиковое.
Следовательно, CPU = 4'277'700/4000=1068 Ядер. RAM = 1068 * 12.5 / 1024 = 13 Гб. Net = 4'277'700 / 1024 / 1024 / 8 = 0.5Гб/с

***call*** - Средняя бизнес логика, примерно 9.000 RPS. CPU = 9000/80 = 113 ядер. RAM = 113 * 125 / 1024 = 14 ГБ. Net: 
Считаем, что 1Мб данных. Тогда, 9000/1024/8 = 1.1Гбит/с

***group*** - Легкое JSON API. Примерно 300.000 RPS, с учетом того, что группы появляются когда пользователь заходит. Тогда,
CPU = 300.000 / 4000 = 75 ядер, RAM = 75 * 12.5 / 1024 = 1 ГБ. Net = 300.000 / 1024 / 1024 / 8 = 0.04 Гбит/с

***message.writer*** = Средняя бизнес логика. RPS пиковое 4.332.984. CPU = 4.332.984 / 80 = 54.162 ядер.
RAM = 54.162 * 125 / 1024 = 6.611 Гб. Net 4.332.984 / 1024 / 1024 / 8 = 0.51 Гбит/с

***message.reader*** = Легкое jsonAPI. RPS пиковое 55.351.280 CPU = 55.351.280 / 4000 = 13.837 ядер. RAM 13.847 * 12.5 / 1024 = 169 Гб
Net - 55.351.280 / 1024 / 1024 / 8 = 6.6 Гбит/с

***file.writer*** = Потоковая передача данных. RPS пиковое 1.896.080 CPU = 1.896.080 / 3000 = 632 Ядра. RAM 632 * 100 / 1024 = 62 Гб
Net = 1.896.080 * 5 / 1024 / 8 = 1.158 Гбит/с

***file.reader*** = Потоковая передача данных. RPS пиковое 24.868.825 CPU = 24.868.825 / 3000 = 8.290 ядер. RAM 8.290 * 100 / 1024 = 810 Гб
Net = 24.868.825 * 5 / 1024 / 8 = 15.178 Гб/с


Значение для определения конфигурации хранилищ были взяты из предыдущих пунктов.

### Конфигурация серверов
Будем считать, что нормальная работа серверов, если они нагружены не более чем на 75%. Исходя из этого, берем запас относительно
всего в 25%. Цена с учетом гарантии 5 лет и RAID 6 для дисков Cassandra, резервирования диска ос RAID 1, защита по питанию. 
Минимальное кол-во серверов на один узел 16шт = 8 ДЦ * 2 резервирование. Цены на own bare metal взяты из [12] Для облачных
решений считал сопоставимые по характеристикам сервера. Из-за неоправданно высокой цены S3 Express One Zone (25 млрд долларов в месяц)
Было принято решение использовать S3 standart(4,5 млн долларов в месяц).

|         Название          |    Хостинг     |                 Конфигурация                  | Cores | Cnt |    Покупка на 5 лет 1шт $    |
|:-------------------------:|:--------------:|:---------------------------------------------:|:-----:|:---:|:----------------------------:|
|    web && mobile nginx    |   own Kuber    | AMD EPYC 5th Gen 9535/264Gb/NVMe 2250Gb/6*100 |  64   | 201 |            20109             |
| web && mobile api gateway |   own Kuber    | AMD EPYC 5th Gen 9535/264Gb/NVMe 2250Gb/6*100 |  64   | 177 |            20109             |
|           auth            |   own Kuber    | AMD EPYC 5th Gen 9175F132Gb/NVMe 2250Gb/2*100 |  16   | 16  |            14000             |
|           user            |   own Kuber    | AMD EPYC 5th Gen 9335132Gb/NVMe 2250Gb/2*100  |  32   | 41  |            15000             |
|           call            |   own Kuber    | AMD EPYC 5th Gen 9175F132Gb/NVMe 2250Gb/2*100 |  16   | 16  |            14000             |
|           group           |   own Kuber    | AMD EPYC 5th Gen 9175F132Gb/NVMe 2250Gb/2*100 |  16   | 16  |            14000             |
|      message.writer       |   own Kuber    | AMD EPYC 5th Gen 9845/264Gb/NVMe 2250Gb/6*100 |  160  | 424 |            25362             |
|      message.reader       |   own Kuber    | AMD EPYC 5th Gen 9535/264Gb/NVMe 2250Gb/6*100 |  64   | 271 |            20109             |
|        file.writer        |   own Kuber    | AMD EPYC 5th Gen 9335/132Gb/NVMe 2250Gb/2*100 |  32   | 25  |            15000             |
|        file.reader        |   own Kuber    | AMD EPYC 5th Gen 9535/264Gb/NVMe 2250Gb/6*100 |  64   | 162 |            20109             |
|           redis           | own bare metal |    AMD Ryzen 7 7700X/432Gb/Nvme 1Tb/210Gb     |   8   | 25  |             5500             |
|         Amazon S3         |     amazon     |                    20500Пб                    |   -   |  -  | 4'500'000*12*5 = 270'000'000 |
|      cassandra group      | own bare metal |    AMD EPYC 7313P/264Gb/NVMe 2250Gb/2*1Gb     |  16   | 16  |             4700             |
|      cassandra call       | own bare metal |    AMD EPYC 7313P/264Gb/NVMe 2250Gb/2*1Gb     |  16   | 16  |             4700             |
|      cassandra user       | own bare metal |    AMD EPYC 7313P/264Gb/NVMe 2250Gb/2*1Gb     |  16   | 16  |             4700             |
|      kafka user2user      | own bare metal | AMD EPYC 5th Gen 9535/264Gb/NVMe 2250Gb/6*100 |  64   | 30  |            20109             |
|    kafka group_message    | own bare metal | AMD EPYC 5th Gen 9535/264Gb/NVMe 2250Gb/6*100 |  64   | 30  |            20109             |
|        kafka file         | own bare metal | AMD EPYC 5th Gen 9535/264Gb/NVMe 2250Gb/6*100 |  64   | 16  |            20109             |
|        kafka video        | own bare metal | AMD EPYC 5th Gen 9535/264Gb/NVMe 2250Gb/6*100 |  64   | 16  |            20109             |
|        kafka photo        | own bare metal | AMD EPYC 5th Gen 9535/264Gb/NVMe 2250Gb/6*100 |  64   | 16  |            20109             |
|        kafka voice        | own bare metal | AMD EPYC 5th Gen 9535/264Gb/NVMe 2250Gb/6*100 |  64   | 16  |            20109             |


Общая стоимость всей системы с учетом на 5 лет, а также что каждый сервер даже при пиковых нагрузках работает на 75% от 
максимальной мощности, составляет:

***COST*** = 20109 * (201 + 177 + 271 + 162 + 30 + 30 + 16 + 16 + 16 + 16) + 14000 * (16 + 16 + 16) + 15000 * (41 + 25) + 25362 + 5500 + 270'000'000 + 4700 * (16 + 16 + 16) = 
***290'720'377 $***
...
[12]:https://www.broadberry.eu/rackmount-servers
