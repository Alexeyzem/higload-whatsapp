...
|        Таблица         |             БД             |           PK            |   Index   |
|:----------------------:|:--------------------------:|:-----------------------:|:---------:|
|        message         |         Cassandra          |   K(sender_id), C(id)   |     -     |   
...
|      message_file      |         Cassandra          |          C(id)          |     -     |   
|     video-metadata     |         Cassandra          |          C(id)          |     -     |
|     photo-metadata     |         Cassandra          |          C(id)          |     -     |
|     file-metadata      |         Cassandra          |          C(id)          |     -     |
| voice-message-metadata |         Cassandra          |          C(id)          |     -     |
...
- ***Алгоритм удаления сообщений после прочтения***
1. Как только пользователь читает сообщение status_worker кладет в Kafka message_id
2. Запрос в cassandra для определения url всех вложений для данного сообщения
3. Запрос на удаление вложений из S3 хранилища всех вложений по полученным выше url
4. Запрос на удаление соответствующих полей в cassandra

- ***Алгоритм удаления сообщений после того, как они не были прочитаны месяц***
1. В S3 можно настроить автоматическое удаление через 1 месяц, так и поступим
2. Удалять из cassandra будем используя Cron. Например, каждую ночь в 1 ночи будет запускаться удаление всех "просроченных" 
сообщений. (Cron-expression: 0 1 * * *)
